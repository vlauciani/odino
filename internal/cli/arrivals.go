...
func runArrivals(ctx context.Context, a *app, query, routeShort string, limit, windowMin int) error {
	...
	// 4. Merge planned + RT.
	rows := mergeArrivals(scheduled, rtLookup, stopID, now, windowEnd, routeShort, a, ctx)
	...
	return output.Table(a.out, []string{"time", "line", "headsign", "min", "src"}, tableRows)
}
...
func mergeArrivals(planned []plannedRow, rt *realtime.TripUpdateLookup, stopID string, now, end time.Time, routeShort string, a *app, ctx context.Context) []arrivalRow {
